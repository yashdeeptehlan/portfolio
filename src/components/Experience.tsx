...
function ExperienceCard({ exp, inView }: { exp: Exp; inView: boolean }) {
  ...
  return (
    <motion.article
      ...
      variants={fadeUp}
      className="holo-card rounded-xl border border-slate-800 bg-[#0e131a]/75 p-6 sm:p-7"
      data-hover
    ...
    <section id="experience" ref={sectionRef} className="py-20 lg:py-28">
      <div className="mx-auto max-w-6xl px-4 sm:px-6 lg:px-8">
        ...
        {certifications.length > 0 && (
          <div className="mt-12">
            <motion.h3
              className="font-mono text-xs tracking-[0.16em] text-slate-400"
              initial="hidden"
              animate={inView ? 'visible' : 'hidden'}
              variants={fadeUp}
            ...
            >
              {certifications.map((cert) => (
                <motion.div
                  key={cert.id}
                  variants={fadeUp}
                  className="rounded-md border border-slate-800 bg-[#0d1218] p-4"
